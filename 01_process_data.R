...
data_collection_start_date_ps = as.Date("15.05.2024", format("%d.%m.%Y")) # Confirm with QAT
data_collection_start_date_cbe = as.Date("15.05.2024", format("%d.%m.%Y")) # Confirm with QAT
# data_collection_end_date = as.Date("16.05.2024",  format("%d.%m.%Y"))
qa_sheet_url_ps = "https://docs.google.com/spreadsheets/d/1FNWkOp_-c6ESm3WeqaKdtfUr0oW08IgH-CQyZdG91UI/edit#gid=1005923888"
...
old_clean_data_path = list.files("output/cleaned_dfs/sterilized/labeled/R3_clean_data/", full.names = T) %>% .[!grepl("/\\~\\$", .)] |> as.list() %>% setNames(gsub(".*Tool (\\d).*", "tool\\1", .))
...
# Read inputs --------------------------------------------------------------
# R3 clean data sets
r3_data.tool0 = read_xlsx_sheets(old_clean_data_path$tool0)
r3_data.tool1 = read_xlsx_sheets(old_clean_data_path$tool1)
r3_data.tool2 = read_xlsx_sheets(old_clean_data_path$tool2)
r3_data.tool3 = read_xlsx_sheets(old_clean_data_path$tool3)
r3_data.tool4 = read_xlsx_sheets(old_clean_data_path$tool4)
r3_data.tool5 = read_xlsx_sheets(old_clean_data_path$tool5)
r3_data.tool6 = read_xlsx_sheets(old_clean_data_path$tool6)
r3_data.tool7 = read_xlsx_sheets(old_clean_data_path$tool7)
r3_data.tool8 = read_xlsx_sheets(old_clean_data_path$tool8)
r3_data.tool9 = read_xlsx_sheets(old_clean_data_path$tool9)
...
length(pending_key_ps) == length(which(qa_sheet_ps$qa_status %in% c("PENDING", "PENDING DUE TO GPS POINT")))
...
length(pending_key_cbe) == length(which(qa_sheet_cbe$qa_status %in% c("PENDING", "PENDING DUE TO GPS POINT")))
...
# Exclude R3 data --------------------------------------------------------- DONE
source("R/exclude_R3_data.R")
